 ...
 */
template <typename BlockT, std::size_t Bits> static constexpr std::size_t remaining() { return Bits % size<BlockT>(); }

/**
 * @brief Returns the number of bits which will not fill one or more \c BlockT
 */
template <typename BlockT> static constexpr std::size_t remaining(const std::size_t bits)
{
...
{
  return whole_blocks<BlockT, Bits>() + (static_cast<BlockT>(remaining<BlockT, Bits>() > 0) * one<BlockT>());
}
